...
def grad_logreg(X, y, w, w_0):
    ...
    ### YOUR CODE BEGINS HERE ###
    y_dash = logistic(np.matmul(X, w) + w_0 * np.ones(m))
    grad_w = np.matmul(y_dash - y, X) / m
    grad_w_0 = np.dot(y_dash - y, np.ones(m)) / m
    ### YOUR CODE ENDS HERE ###
...
def grad_desc(X, y, w, w_0, alpha, n_iter, show_cost=True, show_params=False):
    ...
    for i in range(n_iter):
        ...
        w += - alpha * grad_w       #TODO Update 
        w_0 += - alpha * grad_w_0       #TODO Update 
        # w += 0   #TODO Update 
        # w_0 += 0 #TODO Update
